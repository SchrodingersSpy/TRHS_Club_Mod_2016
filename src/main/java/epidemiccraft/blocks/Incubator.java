...
public class Incubator extends BlockContainer {
	...
	protected Incubator(boolean isActive) {
		super(Material.iron);
		isBurning2 = isActive;
	}
	
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9){
		player.openGui(Constants.MODID, 1, world, x, y, z);
		return true;
	}
	
	public Item getItemDropped(int par1, Random random, int par3){
		return Item.getItemFromBlock(ModBlocks.Incubator);
	}
	...
	public static void updateBlockState(boolean bool, World world, int x, int y, int z)
	{
		...
		if (bool)
		{
			world.setBlock(x, y, z, ModBlocks.IncubatorActive);
		}
		else
		{
			world.setBlock(x, y, z, ModBlocks.Incubator);
		}
		...
		{
			tileentity.validate();
			world.setTileEntity(x, y, z, (TileEntityIncubator) tileentity);
  		}
	}
	...
	public void breakBlock(World world, int x, int y, int z, Block block, int meta){
		if(!isBurning){
			...
			if(tileentityIncubator != null){
				for(int i = 0; i < tileentityIncubator.getSizeInventory(); ++i){
					ItemStack itemstack = tileentityIncubator.getStackInSlot(i);
