...
public class EntityCPoxVillagerMob extends EntityMob{


	private int conversionTime;

	//What are his priorities?
	public EntityCPoxVillagerMob(World par1World) {
		super(par1World);
	...
	protected void dropFewItems(boolean p_70628_1_, int p_70628_2_)
    {
        ...
        for (k = 0; k < j; ++k)
        {
            ...
            {
                this.dropItem(ModItems.TissueCPox, 1);
            }
        }
    }
	...
	public void setCPoxVillager(boolean p_82229_1_)
    {
        this.getDataWatcher().updateObject(13, Byte.valueOf((byte)(p_82229_1_ ? 1 : 0)));
    }
	
	//When an CPoxVillager kills a Villager, this is activated.
	public void onKillEntity(EntityLivingBase p_70074_1_)
    {
        ...
        if ((this.worldObj.difficultySetting == EnumDifficulty.NORMAL || this.worldObj.difficultySetting == EnumDifficulty.HARD) && p_70074_1_ instanceof EntityVillager)
        {
            ...
            EntityCPoxVillagerMob entityCPoxVillager = new EntityCPoxVillagerMob(this.worldObj);
            entityCPoxVillager.copyLocationAndAnglesFrom(p_70074_1_);
            this.worldObj.removeEntity(p_70074_1_);
            entityCPoxVillager.onSpawnWithEgg((IEntityLivingData)null);
            entityCPoxVillager.setCPoxVillager(true);

            this.worldObj.spawnEntityInWorld(entityCPoxVillager);
        }
    }
	
	//When you get attacked by the CPoxVillager, this is activated.
	public boolean attackEntityAsMob(Entity p_70652_1_)
    {
        if (super.attackEntityAsMob(p_70652_1_))
        {
            if (p_70652_1_ instanceof EntityLivingBase)
            {
                ...
                if (((EntityLivingBase)p_70652_1_).isPotionActive(EpidemicCraft.potionCPoxVaccine))
                {	
                	return false;
                }
                ...
                else
                {
                    ...
                    ((EntityLivingBase)p_70652_1_).addPotionEffect(new PotionEffect(EpidemicCraft.potionCPox.id, b0 * 1200, 0));
	...
	//Start converting the CPoxVillager into a normal CPoxVillager.
    ...
    //What happens when you stick the CPoxVillager with a Syringe?
    @SideOnly(Side.CLIENT)
    public void handleHealthUpdate(byte p_70103_1_)
    {
        if (p_70103_1_ == 16)
        {
            this.worldObj.playSound(this.posX + 0.5D, this.posY + 0.5D, this.posZ + 0.5D, "mob.villager.hurt", 1.0F + this.rand.nextFloat(), this.rand.nextFloat() * 0.7F + 0.3F, false);
        }
        else
        {
	...
	//What happens when you right click the CPoxVillager with a syringe?
	public boolean interact(EntityPlayer p_70085_1_)
    {
        ItemStack itemstack = p_70085_1_.getCurrentEquippedItem();

        if (itemstack != null && itemstack.getItem() == ModItems.SyringeCPox && itemstack.getItemDamage() == 0)
        {
	...
	//Is the CPoxVillager Converting?
	...
	//What happens to the CPoxVillager's AI when the CPoxVillager finishes converting?
	protected void convertToCPoxVillager()
    {
        EntityVillager entityCPoxVillager = new EntityVillager(this.worldObj);
        entityCPoxVillager.copyLocationAndAnglesFrom(this);
        entityCPoxVillager.onSpawnWithEgg((IEntityLivingData)null);

        this.worldObj.removeEntity(this);
        this.worldObj.spawnEntityInWorld(entityCPoxVillager);
        entityCPoxVillager.addPotionEffect(new PotionEffect(Potion.confusion.id, 200, 0));
    }
	
	//If we run this, the CPoxVillager turns into a CPoxVillager.
	...
	//What does the CPoxVillager say while idling?
	protected String getLivingSound()
    {
        return "mob.villager.hurt";
    }

	//What does the CPoxVillager say while in pain?
	protected String getHurtSound()
    {
        return "mob.villager.hurt";
    }
	
	//What does the CPoxVillager say when he dies?
